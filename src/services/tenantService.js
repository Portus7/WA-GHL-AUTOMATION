...
async function registerNewTenant(locationId, companyId, initialStatus = 'active', subscriptionId = null, locationName = null, agencyName = null) {
    try {
        ...
        const sql = `
            INSERT INTO tenants (location_id, status, trial_ends_at, plan_id, settings, created_at, agency_id, linked_subscription_id, name, agency_name)
            VALUES ($1, $2, $3, $4, $5::jsonb, NOW(), $6, $7, $8, $9)
            ON CONFLICT (location_id) 
            DO UPDATE SET 
                status = EXCLUDED.status, -- Actualizamos al estado que enviamos
                agency_id = EXCLUDED.agency_id,
                name = COALESCE(EXCLUDED.name, tenants.name),
                agency_name = COALESCE(EXCLUDED.agency_name, tenants.agency_name),
                updated_at = NOW()
        `;

        // Pasamos initialStatus en lugar de hardcodear 'active'
        await pool.query(sql, [locationId, initialStatus, trialEnd, planId, JSON.stringify(defaultSettings), companyId, subscriptionId, locationName, agencyName]);
